...
var rt_life_cycle = {
  ...
  $RTModule: {
    ...
    sendMessage: function(gameObject, method, parameter) {
      gameObject = gameObject || "RTLifeCycle";
      Module.SendMessage(gameObject, method, parameter);
    },
    ...
    onCallbackNoParameter: function(gameObject, callbackId, callbackType, method) {
      method = method || "_onCallbackNoParameter";
    ...
    onCallbackParameter: function(gameObject, callbackId, callbackType, method, resObj) {
      resObj._callbackId = callbackId;
      resObj._callbackType = callbackType;
      RTModule.sendMessage(gameObject, method, JSON.stringify(resObj));
    },

    handleCallbackNoParameter: function(gameObject, callbackId, method) {
      return {
        success: function() {
          RTModule.onCallbackNoParameter(gameObject, callbackId, 0, method);
        },
        fail: function() {
          RTModule.onCallbackNoParameter(gameObject, callbackId, 1, method);
        },
        complete: function() {
          RTModule.onCallbackNoParameter(gameObject, callbackId, 2, method);
        }
      };
    },

    handleCallbackParameter: function(gameObject, callbackId, method, onSuccess, onFail) {
      return {
        success: function(res) {
          if (onSuccess) {
            res = onSuccess(res);
          }
          RTModule.onCallbackParameter(gameObject, callbackId, 0, method, res);
        },
        fail: function(res) {
          if (onFail) {
            res = onFail(res);
          }
          RTModule.onCallbackParameter(gameObject, callbackId, 1, method, res);
        },
        complete: function() {
          RTModule.onCallbackNoParameter(gameObject, callbackId, 2, method);
        }
  ...
  $RTLifeCycleModule: {
    ...
    onHide: function() {
      RTModule.sendMessage(undefined, "_onHideCallback");
    },

    onShow: function() {
      RTModule.sendMessage(undefined, "_onShowCallback");
    }
  ...
  RTInit: function() {
    if (!RTModule.runtime) {
      if (typeof loadRuntime !== "function") {
        console.error("loadRuntime() not exist");
        return;
      }
      RTModule.runtime = loadRuntime();
      RTFSMModule.init();
    }
  },

  RTExitApplication: function (callbackId, data) {
    ...
    RTModule.runtime.exitApplication(args);
  },

  RTGetLaunchOptionsSync: function() {
    var res = RTModule.runtime.getLaunchOptionsSync();
    if (res === undefined) {
      res = "";
    } else if (typeof res === "object") {
      res = JSON.stringify(res);
    }
    return RTModule.stringToBuffer(res);
  },

  RTOnHide: function() {
    RTModule.runtime.onHide(RTLifeCycleModule.onHide);
  },

  RTOffHide: function() {
    RTModule.runtime.offHide(RTLifeCycleModule.onHide);
  },

  RTOnShow: function() {
    RTModule.runtime.onShow(RTLifeCycleModule.onShow);
  },

  RTOffShow: function() {
    RTModule.runtime.offShow(RTLifeCycleModule.onShow);
  }

};

autoAddDeps(rt_life_cycle, "$RTModule");
autoAddDeps(rt_life_cycle, "$RTLifeCycleModule");
mergeInto(LibraryManager.library, rt_life_cycle);
