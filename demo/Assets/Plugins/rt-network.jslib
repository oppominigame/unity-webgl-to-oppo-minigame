  ...
  $RTNetworkModule: {
    ...
    onNetworkStatusChange: function(res) {
      RTModule.sendMessage(
        "RTNetwork",
        "_onNetworkStatusChangeCallback",
        JSON.stringify(res));
    }

  },

  RTGetNetworkType: function(callbackId) {
    var args = RTModule.handleCallbackParameter(
      "RTNetwork",
      Pointer_stringify(callbackId),
      "_onGetNetworkType");
    RTModule.runtime.getNetworkType(args);
  },

  RTOnNetworkStatusChange: function() {
    RTModule.runtime.onNetworkStatusChange(RTNetworkModule.onNetworkStatusChange);
  },

  RTOffNetworkStatusChange: function() {
    RTModule.runtime.offNetworkStatusChange(RTNetworkModule.onNetworkStatusChange);
  }

};

autoAddDeps(rt_network, "$RTNetworkModule");
mergeInto(LibraryManager.library, rt_network);
